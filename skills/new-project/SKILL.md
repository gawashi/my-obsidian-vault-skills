...
- 当日日付と現在時刻を取得する（`date "+%F %H:%M"`）。`{当日}`=`YYYY-MM-DD`、`{時刻}`=`HH:MM`。`{時刻}` は実行時の値を使い、ハードコードしない（frontmatter は `{当日}` のみで時刻は付けない）。
...
- 作業したら → **LOG.md** の「現状 / 次の一手」を更新し「ログ」に `- {当日} {時刻}: 要点` を1行（新しいものを上に）。
