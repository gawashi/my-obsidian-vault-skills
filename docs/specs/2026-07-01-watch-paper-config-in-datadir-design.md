...
| コード内の言語 | **コメント・docstring・stderr メッセージはすべて英語**（既存コードベースの慣習に一致）。SKILL.md・README.md の散文は日本語のまま。config の値（`name` 等）はユーザーデータなので対象外 |
...
```python
TEMPLATE_PATH = Path(__file__).parent / "config.example.json"   # bundled template (copy source)
...
def ensure_config(data_dir):
    ...
    Returns (path, created):
      created=True  ... template was copied to create a new config (caller should stop)
      created=False ... an existing config.json is present (read it as-is)
    Raises OSError if the template cannot be read/copied (caller returns rc=2).
    """
    dst = config_path(data_dir)
    if dst.exists():
        return dst, False
    shutil.copyfile(TEMPLATE_PATH, dst)     # data_dir already mkdir'd by setup_data_dir
...
if created:
    print(f"[watch-paper] created {cfg_path} from template; "
          f"edit the themes and re-run.", file=sys.stderr)
    return 0   # do not fetch with the example themes
